...
if uploaded_file is not None:
    ...
    original_grouped_df = filtered_data.groupby(['type', 'category1', 'category2', 'category3', 'category4']).agg(
    ...
    grouped_df = filtered_data.groupby(['day']).agg(
        average_in_mbps=('in_mbps', 'mean'),
        average_out_mbps=('out_mbps', 'mean')
    ).reset_index()
    ...
    weekday_grouped_df = filtered_data.groupby(['weekday2']).agg(
        average_in_mbps=('in_mbps', 'mean'),
        average_out_mbps=('out_mbps', 'mean')
    ...
    hourly_grouped_df = filtered_data.groupby(['hour']).agg(
        average_in_mbps=('in_mbps', 'mean'),
        average_out_mbps=('out_mbps', 'mean')
    ).reset_index()
